...
- **Версия мода рядом с версией приложения** — клик открывает «Что нового» на вкладке мода: изменения из релизов на GitHub; вкладки переключают список изменений Яндекс Музыки и мода. Точка на плашке — доступно обновление
...
- **Автопауза** — при блокировке компьютера (и продолжение после входа) и при отключении наушников
...
- **Mica и Acrylic** — настоящая прозрачность окна Windows 11: сквозь окно размыт рабочий стол
- **Акцент по обложке** — жёлтые кнопки и переключатели перекрашиваются в цвет обложки текущего трека
...
- **Виджет «Сейчас играет» для OBS** и **локальный API** — трек в JSON, поток изменений и управление плеером из своих программ
...
<details>
<summary><b>Виджет для OBS и локальный API</b></summary>

Включите **Настройки → Моды → Интеграции → Локальный API и виджет для OBS**. Сервер работает только на этом компьютере (`127.0.0.1`), по умолчанию на порту `24850`.

**Виджет:** в OBS добавьте источник **«Браузер»** с адресом `http://127.0.0.1:24850/widget` (например, 600×140). Параметры в адресе:

| Параметр | Значения |
|---|---|
| `layout` | `card` (по умолчанию) · `bar` — узкая строка · `minimal` — только текст |
| `hidePaused` | `1` — прятать виджет, пока музыка на паузе |
| `accent` | `cover` (цвет обложки) · `yellow` · любой цвет, например `ff5500` |
| `align` | `left` · `right` |
...
Команды: `playPause`, `play`, `pause`, `next`, `prev`, `like`, `dislike`, `shuffle`, `repeat`, `volumeUp`, `volumeDown`. Токен — в настройках рядом с API. Сайты в браузере не могут ни читать состояние, ни отправлять команды: ответы без CORS, чужой `Host` отклоняется, команды только с токеном.
...
| `localApi` / `localApiPort` | `false` / `24850` | локальный API и виджет для OBS |
...
modloader/            код мода (копируется в %APPDATA%\YandexMusic\modloader)
  ...
  localapi.js           локальный HTTP API (127.0.0.1) и раздача виджета
  widget.html           виджет «Сейчас играет» для OBS
